# ============================================================
# scripts/train_basic_gru.py —— 基础 GRU 模型训练脚本
# ============================================================
# TODO 2.1 的核心脚本：在 FD001 数据集上训练一个基础 GRU
# 作为 LSTM 的对比基线模型，验证 GRU vs LSTM 在 RUL 预测上的优劣
#
# 功能:
...
#   6. 训练完成后保存最佳模型到 saved_models/retrained_20260826/
#   7. 记录训练日志到 logs/
#
# ⚠️ 注意：数据加载方式与验证集划分方式
#   与 Parameter-matched LSTM 保持一致，确保公平对比。
# ============================================================
...
from core_models.base_models import GRUModel
...
def load_data(subset='FD001', processed_dir='data/processed', val_ratio=0.2):
    ...
    参数:
        subset:         数据集编号（TODO 2.1 仅用 FD001）
        processed_dir: 预处理数据目录
        val_ratio:      验证集比例

    返回:
        train_loader, val_loader: 训练和验证 DataLoader
        (X_test, y_test):         测试集 tensor（用于最终评估）
    """
...
def save_checkpoint(model, optimizer, epoch, best_loss, train_losses, val_losses,
                    filepath='saved_models/retrained_20260826/original_paper_static/baselines/gru_checkpoint.pt'):
    """
    保存训练状态，支持断点续训
...
# ============================================================
def load_checkpoint(model, optimizer, filepath='saved_models/retrained_20260826/original_paper_static/baselines/gru_checkpoint.pt'):
    """
    从 checkpoint 恢复训练状态
    ...
    返回:
        start_epoch: 从哪一轮继续
        best_loss:   历史最佳验证损失
        train_losses, val_losses: 历史损失记录
        若文件不存在则返回初始值
    """
...
def validate(model, dataloader, loss_fn, device):
    ...
    返回:
        avg_loss: 平均损失
        y_pred_all, y_true_all: 全部预测值和真实值（numpy）
    """
    model.eval()
    total_loss = 0.0
    num_batches = len(dataloader)
...
def train(model, train_loader, val_loader, loss_fn, optimizer, device,
          ...
          resume=False, checkpoint_path='saved_models/retrained_20260826/original_paper_static/baselines/gru_checkpoint.pt'):
    """
    GRU 模型主训练循环
    ...
    参数:
        ...
        val_loader:     验证数据 DataLoader（从训练集切分的 80/20）
        loss_fn:        损失函数
        optimizer:      优化器
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if val_loss < best_loss:
            ...
            # 保存最佳模型
            best_model_path = 'saved_models/retrained_20260826/original_paper_static/baselines/gru_best_FD001.pt'
...
def save_training_log(train_losses, val_losses, val_rmses, val_nasa_scores, subset='FD001'):
    """
    将训练过程的损失记录保存为 JSON 文件，方便后续画图分析
    ...
    log_path = f'logs/gru_{subset}_{timestamp}.json'
...
def final_evaluate(model, X_test, y_test, loss_fn, device, batch_size=BATCH_SIZE):
    """
    在测试集上做最终评估，输出 RMSE 和 NASA Score
...
if __name__ == '__main__':
    ...
    # ---- 1. 加载数据（训练集内 80/20 切分 train/val，与 LSTM 完全一致）----
    train_loader, val_loader, (X_test, y_test) = load_data(
        subset='FD001', val_ratio=0.2
    )

    # ---- 2. 创建 GRU 模型（超参数与 BasicLSTM 完全一致）----
    model = GRUModel(
        input_dim=NUM_FEATURES,   # 17
        hidden_dim=128,           # 与 LSTM 相同
        num_layers=3,             # 与 LSTM 相同
        dropout=0.3               # 与 LSTM 相同
    ).to(device)

    print(f"\n🔧 模型: GRUModel")
    print(f"  参数量: {sum(p.numel() for p in model.parameters()):,}")

    # ---- 3. 定义损失函数和优化器（与 LSTM 完全一致）----
    ...
    # ---- 4. 训练（在 val_loader 上做早停和选模型）----
    # 设置 resume=True 可以从上次中断处继续训练
    try:
    ...
    best_checkpoint = torch.load('saved_models/retrained_20260826/original_paper_static/baselines/gru_best_FD001.pt')
