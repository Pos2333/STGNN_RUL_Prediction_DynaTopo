# ============================================================
# scripts/train_basic.py —— STGNN 主线模型训练脚本
# ============================================================
# TODO 3 的核心脚本：在 FD001 数据集上训练完整 STGNN 模型
# (MSTCN + GAT + Transformer)
#
# 功能:
...
def save_checkpoint(model, optimizer, epoch, best_loss,
                    ...
                    filepath='saved_models/stgnn_checkpoint.pt'):
    """保存训练状态，支持断点续训"""
    torch.save({
        'epoch': epoch,
...
# ============================================================
def load_checkpoint(model, optimizer, filepath='saved_models/stgnn_checkpoint.pt'):
    """从 checkpoint 恢复训练状态"""
...
def train(model, train_loader, val_loader, loss_fn, optimizer, edge_index, device,
          ...
          resume=False, checkpoint_path='saved_models/stgnn_checkpoint.pt'):
    """STGNN 主训练循环"""
    # ---- 尝试恢复训练 ----
    if resume:
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if val_loss < best_loss:
            ...
            best_model_path = 'saved_models/stgnn_best_FD001.pt'
            torch.save({
                'model_state_dict': model.state_dict(),
                'best_loss': best_loss,
                'epoch': epoch,
...
def save_training_log(train_losses, val_losses, subset='FD001'):
    ...
    log_path = f'logs/stgnn_{subset}_{timestamp}.json'
...
if __name__ == '__main__':
    print("=" * 60)
    print("  🧪 STGNN 主线模型训练 —— TODO 3")
    print("  MSTCN + GAT + Transformer")
    print("=" * 60)
    ...
    print(f"\n🔧 模型: STGNN (MSTCN + GAT + Transformer)")
    ...
    best_checkpoint = torch.load('saved_models/stgnn_best_FD001.pt')
    ...
    print(f"\n🎉 TODO 3 完成！STGNN 主线模型训练完毕。")
    print(f"  最佳模型保存在: saved_models/stgnn_best_FD001.pt")
