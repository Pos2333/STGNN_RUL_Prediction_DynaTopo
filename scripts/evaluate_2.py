# ============================================================
# scripts/evaluate_2.py —— 跨工况迁移实验分析
# ============================================================
# TODO 5 动作2：对比"带迁移学习"与"不带迁移学习"的 STGNN
# 在 FD002、FD003、FD004 三个目标数据集上的预测性能
#
# 对比方案：
#   A. 无迁移: 用 FD001 训练的 STGNN 直接测试各目标域
#   B. 有迁移: 用 FD001→目标域 迁移训练的 STGNN 测试
#
# 用法：
#   python scripts/evaluate_2.py
# ============================================================
...
def run_evaluation(target_subsets=None):
    ...
    for target in target_subsets:
        ...
        pretrain_path = 'saved_models/stgnn_best_FD001.pt'
        ...
        # ---- B. 有迁移：FD001→target 迁移模型 ----
        transfer_path = f'saved_models/transfer_best_{target}.pt'
        ...
        results[target] = {
            ...
            'with_transfer': {'rmse': rmse_b, 'score': score_b},
        }
    ...
    print(f"  📊 跨工况迁移实验汇总")
    print(f"{'='*70}")
    print(f"  {'数据集':<10} {'方法':<15} {'RMSE':>10} {'NASA Score':>15}")
    print(f"  {'-'*50}")

    for target, res in results.items():
        if res['no_transfer']['rmse'] is not None:
            print(f"  {target:<10} {'无迁移':<15} "
                  f"{res['no_transfer']['rmse']:>10.2f} "
                  f"{res['no_transfer']['score']:>15.2f}")
        if res['with_transfer']['rmse'] is not None:
            print(f"  {target:<10} {'有迁移(LMMD)':<15} "
                  f"{res['with_transfer']['rmse']:>10.2f} "
                  f"{res['with_transfer']['score']:>15.2f}")
    ...
    for target, res in results.items():
        json_results[target] = {
            ...
            'with_transfer': {
                'rmse': float(res['with_transfer']['rmse']) if res['with_transfer']['rmse'] is not None else None,
                'score': float(res['with_transfer']['score']) if res['with_transfer']['score'] is not None else None,
            },
        }
