# ============================================================
# scripts/ablation_dynatopo.py —— A2B2 组件消融实验（UDA 场景，FD001→FD002）
# ============================================================
# 消融变体：
...
def train_uda(model, src_tr, tgt_tr, tgt_va, loss_fn, src_edge, tgt_edge, optimizer,
    ...
    for epoch in range(num_epochs):
        ...
        # 目标域验证（使用验证集 tgt_va，与 train_transfer.py 一致）
        model.eval()
        tgt_loss = 0.0
        with torch.no_grad():
            for X_b, y_b in tgt_va:
                X_b, y_b = X_b.to(device), y_b.to(device)
                pred = model(X_b, tgt_edge.to(device))
                tgt_loss += loss_fn(pred, y_b).item() * X_b.size(0)
        tgt_loss /= len(tgt_va.dataset)
...
def main():
    ...
    for label, preset in ABLATION_PRESETS.items():
        ...
        model = train_uda(model, src_tr, tgt_tr, tgt_va, loss_fn,
    ...
    print(f"{'='*80}")
    print(f"  {'变体':<20} {'test RMSE':>10} {'test NASA':>12} {'参数量':>10}")
    print(f"  {'─'*52}")
