# ============================================================
# scripts/train_transfer_v2.py —— 跨工况迁移学习训练脚本（v2：无 Transformer）
# ============================================================
# TODO 5 动作1：基于 FD001 预训练模型（无 Transformer 变体），
# 分别迁移到 FD002 / FD003 / FD004
#
# 策略：
...
#
# 用法：
#   python scripts/train_transfer_v2.py                      # 训练全部目标域
#   python scripts/train_transfer_v2.py --target FD002       # 仅训练 FD002
#   python scripts/train_transfer_v2.py --target FD003       # 仅训练 FD003
#   python scripts/train_transfer_v2.py --target FD004       # 仅训练 FD004
#   python scripts/train_transfer_v2.py --target FD002 --resume  # 续训
# ============================================================
...
def train_transfer(model, src_train_loader, src_val_loader,
                   ...
                   resume=False,
                   checkpoint_path='saved_models/transfer_v2_checkpoint.pt'):
    """
    迁移学习主训练循环
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if tgt_val_loss < best_tgt_val_loss:
            ...
            best_model_path = f'saved_models/transfer_v2_best_{target_subset}.pt'
            torch.save({
...
def save_training_log(train_losses, val_losses, source='FD001', target='FD002'):
    ...
    log_path = f'logs/transfer_v2_{source}_to_{target}_{timestamp}.json'

    log_data = {
        'model': 'STGNN_Transfer_v2',
        'source': source,
        'target': target,
        'lmmd_lambda': LMMD_LAMBDA,
        'tgt_task_weight': TGT_TASK_WEIGHT,
...
def train_single_target(source_subset, target_subset, device, resume=False):
    ...
    checkpoint_path = f'saved_models/transfer_v2_checkpoint_{target_subset}.pt'
...
if __name__ == '__main__':
    ...
    print(f"\n{'='*60}")
    print(f"  🎉 全部迁移学习训练完成！")
    print(f"  模型保存在: saved_models/transfer_v2_best_*.pt")
    print(f"{'='*60}")
