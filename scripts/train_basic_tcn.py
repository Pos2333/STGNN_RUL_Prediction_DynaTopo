# ============================================================
# scripts/train_basic_tcn.py —— 标准 TCN 模型训练脚本
# ============================================================
# TODO 2.2 的核心脚本：在 FD001 数据集上训练一个标准 TCN
# 作为 MSTCN 的对比基线，证明多尺度分支的有效性
#
# 功能:
#   1. 加载 data/processed 中预处理好的 FD001 数据
#   2. 使用 TCNModel（单尺度膨胀因果卷积）进行训练
...
def load_data(subset='FD001', processed_dir='data/processed', val_ratio=0.2):
    ...
    参数:
        subset:         数据集编号（TODO 2.2 仅用 FD001）
...
def save_checkpoint(model, optimizer, epoch, best_loss, train_losses, val_losses,
                    filepath='saved_models/retrained_20260826/original_paper_static/baselines/tcn_checkpoint.pt'):
    """保存训练状态，支持断点续训"""
...
# ============================================================
def load_checkpoint(model, optimizer, filepath='saved_models/retrained_20260826/original_paper_static/baselines/tcn_checkpoint.pt'):
    """从 checkpoint 恢复训练状态"""
...
def train_one_epoch(model, dataloader, loss_fn, optimizer, device):
    ...
    for X_batch, y_batch in dataloader:
        ...
        # 前向传播（TCNModel 内部自动 permute [B,W,N]→[B,N,W]）
...
def train(model, train_loader, val_loader, loss_fn, optimizer, device,
          ...
          resume=False, checkpoint_path='saved_models/retrained_20260826/original_paper_static/baselines/tcn_checkpoint.pt'):
    """TCN 模型主训练循环"""
    # ---- 尝试恢复训练 ----
    if resume:
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if val_loss < best_loss:
            ...
            best_model_path = 'saved_models/retrained_20260826/original_paper_static/baselines/tcn_best_FD001.pt'
...
if __name__ == '__main__':
    ...
    # ---- 1. 加载数据（训练集内 80/20 切分 train/val，与 LSTM/GRU 完全一致）----
    ...
    model = TCNModel(
        ...
        num_channels=64,          # 与 MSTCN 时序部分可比
        kernel_size=3,            # 单尺度，用于对比多尺度
        num_layers=4,             # dilation=[1,2,4,8]，感受野=31>30
        dropout=0.3
    ).to(device)
    ...
    best_checkpoint = torch.load('saved_models/retrained_20260826/original_paper_static/baselines/tcn_best_FD001.pt')
