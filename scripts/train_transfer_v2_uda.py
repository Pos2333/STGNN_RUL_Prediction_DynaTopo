# ============================================================
# scripts/train_transfer_v2_uda.py —— 无监督域自适应训练脚本（v2：无 Transformer）
# ============================================================
# TODO 5: 基于 FD001 预训练模型（无 Transformer 变体），
# 采用无监督域自适应 (UDA) 策略迁移到 FD002 / FD003 / FD004
#
# UDA 策略（与半监督版本的区别）：
#   - 目标域标签不参与任务损失计算
#   - 仅依靠 LMMD 拉近源域和目标域的特征分布
#   - 总损失 = 源域任务损失 + λ * LMMD
#
# 流程：
#   1. 加载 FD001 上训练好的 STGNN v2 作为预训练起点
#   2. 同时加载 FD001（源域）和 目标域 数据
#   3. 仅源域计算 CombinedLoss（MSE + NASA Score）
#   4. 对源域和目标域的中间特征计算 LMMD 损失（双向子域对齐）
#   5. 总损失 = 源域任务损失 + λ * LMMD
#   6. 早停基于目标域验证损失（与半监督版保持一致，确保公平）
#   7. 支持 checkpoint 暂停恢复
#
# 用法：
#   python scripts/train_transfer_v2_uda.py                      # 训练全部目标域
#   python scripts/train_transfer_v2_uda.py --target FD002       # 仅训练 FD002
#   python scripts/train_transfer_v2_uda.py --target FD002 --resume  # 续训
# ============================================================
...
# ============================================================
def train_one_epoch_uda(model, src_loader, tgt_loader,
                         task_loss_fn, src_edge, tgt_edge, optimizer,
                         device, lmmd_lambda=LMMD_LAMBDA):
    """
    无监督域自适应 (UDA) 训练一个 epoch

    核心：
      - 仅源域计算任务损失（目标域标签不参与监督）
    ...
    for _ in range(num_batches):
        ...
        # ---- 任务损失（UDA：仅源域监督，目标域标签不参与） ----
        loss_src = task_loss_fn(y_pred_src, y_src)
        # 注意：UDA 策略下目标域标签不用于任务损失
        # 仅靠 LMMD 对齐特征分布来实现知识迁移
        task_loss = loss_src
    ...
    n = num_batches
    return total_task_src / n, total_lmmd / n, total_loss_sum / n
...
# 5. 主训练函数
# ============================================================
def train_uda(model, src_train_loader, src_val_loader,
              tgt_train_loader, tgt_val_loader,
              ...
              resume=False,
              checkpoint_path='saved_models/transfer_v2_uda_checkpoint.pt'):
    """
    无监督域自适应 (UDA) 主训练循环

    与半监督版一致的早停机制：基于目标域验证损失
    """
    if resume:
    ...
    print(f"\n{'='*60}")
    print(f"  🚀 开始无监督域自适应 (UDA) 训练 ({target_subset})")
    print(f"  设备: {device}, 最大轮数: {num_epochs}, 早停: {patience}")
    print(f"  LMMD λ: {lmmd_lambda} (目标域标签不参与任务损失)")
    print(f"{'='*60}")
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        # ---- 训练 ----
        task_src, lmmd_val, total_loss = train_one_epoch_uda(
        ...
        if tgt_val_loss < best_tgt_val_loss:
            ...
            best_model_path = f'saved_models/transfer_v2_uda_best_{target_subset}.pt'
            torch.save({
    ...
    print(f"\n{'='*60}")
    print(f"  ✅ UDA 训练完成！最佳目标域验证损失: {best_tgt_val_loss:.4f}")
    print(f"{'='*60}")
...
def save_training_log(train_losses, val_losses, source='FD001', target='FD002'):
    ...
    log_path = f'logs/transfer_v2_uda_{source}_to_{target}_{timestamp}.json'
...
def build_and_load_model(source_subset, device, resume, pretrain_path):
    """
    构建 STGNN v2 模型，加载 FD001 预训练权重（UDA 版本）
...
def train_single_target(source_subset, target_subset, device, resume=False):
    """训练 FD001 → target 的 UDA 迁移模型"""

    # 每个目标域使用独立的 checkpoint
    checkpoint_path = f'saved_models/transfer_v2_uda_checkpoint_{target_subset}.pt'
    pretrain_path = f'saved_models/stgnn_v2_best_{source_subset}.pt'
    ...
    print(f"  策略: 无监督域自适应 (UDA)")
    print(f"  任务损失: CombinedLoss (MSE×{MSE_WEIGHT} + NASA×{NASA_SCORE_WEIGHT}) [仅源域]")
    print(f"  LMMD λ: {LMMD_LAMBDA}")
    print(f"  优化器: Adam (lr={LEARNING_RATE * 0.1})")

    # ---- 训练 ----
    model, train_losses, tgt_val_losses = train_uda(
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='无监督域自适应 (UDA) 训练 (v2: 无 Transformer)')
    ...
    print("=" * 60)
    print(f"  🧪 无监督域自适应 (UDA) 训练 (v2: 无 Transformer) —— TODO 5")
    print(f"  {source_subset} (源域) → {target_list} (目标域)")
    print(f"  策略: 仅源域监督 + LMMD 双向子域对齐")
    print("=" * 60)
    ...
    for target_subset in target_list:
        ...
        print(f"🎉 {source_subset} → {target_subset} UDA 训练完成！")

    print(f"\n{'='*60}")
    print(f"  🎉 全部 UDA 训练完成！")
    print(f"  模型保存在: saved_models/transfer_v2_uda_best_*.pt")
    print(f"{'='*60}")
