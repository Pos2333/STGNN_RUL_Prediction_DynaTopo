# ============================================================
# scripts/train_basic_lstm.py —— 基础 LSTM 模型训练脚本
# ============================================================
# TODO 2 的核心脚本：在 FD001 数据集上训练一个基础 LSTM
# 作为 RUL 预测的基线模型，后续 STGNN 模型将与此对比
#
# 功能:
#   1. 加载 data/processed 中预处理好的 FD001 数据
#   2. 使用 BasicLSTM 模型进行训练
...
def save_checkpoint(model, optimizer, epoch, best_loss, train_losses, val_losses,
                    filepath='saved_models/lstm_checkpoint.pt'):
    """
    保存训练状态，支持断点续训
...
# ============================================================
# 4. 加载 checkpoint（恢复训练）
# ============================================================
def load_checkpoint(model, optimizer, filepath='saved_models/lstm_checkpoint.pt'):
    """
    从 checkpoint 恢复训练状态
...
def train(model, train_loader, val_loader, loss_fn, optimizer, device,
          ...
          resume=False, checkpoint_path='saved_models/lstm_checkpoint.pt'):
    """
    LSTM 模型主训练循环
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if val_loss < best_loss:
            ...
            # 保存最佳模型
            best_model_path = 'saved_models/lstm_best_FD001.pt'
            torch.save({
                'model_state_dict': model.state_dict(),
                'best_loss': best_loss,
                'epoch': epoch,
...
def save_training_log(train_losses, val_losses, subset='FD001'):
    ...
    log_path = f'logs/lstm_{subset}_{timestamp}.json'
...
if __name__ == '__main__':
    ...
    # ---- 1. 加载数据（训练集内 80/20 切分 train/val） ----
    ...
    best_checkpoint = torch.load('saved_models/lstm_best_FD001.pt')
